...
15, 15, 10, and 15 metres, respectively, while Mirko will take 5 metres off the first tree and 2 metres off the fourth tree 
...
bool isSuffWood(int k, int n, int m){
	ll wood=0;
	loop(i,n) if(tree[i]>=k) wood+= (tree[i]-k);
	return wood>=m;
}
...
void binarySearch() {
    ...
    while (low <= high) {
        ...
        if (isSuffWood(mid, n, m)) low = mid+1; 
        else high = mid - 1;
    }
    cout << high; 
}
